...
**The count is derived, never remembered.** It is the number of reviews on the PR by a required reviewer, re-read every wake. This is not an implementation detail; it is most of why the runaway was invisible. A session that tracks the tally loses it to compaction, and the next session sees one review comment, a rule telling it to answer, and no evidence that twelve rounds came before. Every wake honestly looks like the first one. Anything the driver must not lose has to be re-derivable or written down, and a round count is cheap to re-derive.
