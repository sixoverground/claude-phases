...
* **`continue` writes a baseline.** It records the current count in the row's `Note` as `review-baseline: N`, and later counts subtract it. A derived count still reads above the threshold the moment after someone says carry on, so without the baseline the stop could never be cleared.
